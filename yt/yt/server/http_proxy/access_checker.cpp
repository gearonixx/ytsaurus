...
private:
    ...
    // ● std::atomic<T> работает только для маленьких типов (int, указатель) — потому что процессор умеет атомарно прочитать/записать 8 байт одной инструкцией. std::string внутри — указатель на кучу + размер +
    // capacity, ~24 байта, никакая CPU-инструкция это разом не обновит. Поэтому std::atomic<std::string> не скомпилится. TAtomicObject<std::string> обходит это: внутри спинлок, при чтении/записи кратко лочится —
    //  снаружи выглядит как атомик.

  //   Потому что OnProxyRoleUpdated пишет в это поле из потока координатора, а CheckAccess читает его параллельно из потоков, обрабатывающих HTTP-запросы. Без синхронизации это data race — UB. TAtomicObject даёт
  // безопасные Store/Load.
    ...
    // Статический конфиг читается один раз при старте процесса из файла и дальше не меняется — чтобы изменить,
    // надо рестартовать сервис.Статический конфиг читается один раз при старте процесса из файла и дальше не меняется — чтобы изменить,
    // надо рестартовать сервис. Динамический подгружается на лету из централизованного хранилища (в YT это Cypress — их распределённая ФС/мета-стор),
    // периодически опрашивается и применяется без рестарта через коллбэки типа OnDynamicConfigChanged.
    void OnProxyRoleUpdated(const std::string& newRole)
    {
        // YT-макрос: проверяет, что метод можно звать из любого потока (в дебаге это assert, в релизе — пусто). Тут он говорит «OnProxyRoleUpdated thread-safe, координатор может позвать его откуда угодно». Без такой
        // пометки в YT принято указывать конкретный поток-владелец через YT_ASSERT_THREAD_AFFINITY(SomeThread).

        YT_ASSERT_THREAD_AFFINITY_ANY();

        // ● Чтобы закэшированное ProxyRole_ в TAccessChecker не осталось со старым значением — его читают другие места (логи, метрики, потенциально проверки), и они должны видеть актуальную роль. Сигнал — единственное
        // уведомление о смене, без подписки checker не узнал бы никогда.
        ...
        const TProxyDynamicConfigPtr& newConfig)
    {
        ...
        // «Взять `Enabled` из нового динамического конфига, а если оно не задано — взять `Enabled` из статического конфига.»
