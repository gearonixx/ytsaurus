...
// это другое
// Thread-local «состояние потока» автомата мастера. Хранит флаги «я внутри мутации / в teardown», ссылку на bootstrap и эпоху. Плюс две очереди объектов, которым надо уменьшить refcount.
//
// Уменьшают refcount не сразу, а через очередь — потому что FlushObjectUnrefs сортирует их по ObjectId и применяет в этом порядке. Это нужно, чтобы лидер и фолловеры Hydra сделали ровно одни и те же действия
// → state не разъедется.
//
////////////////////////////////////////////////////////////////////////////////
///
    ///- TTableNode — динамические таблицы (sorted/ordered);
    // - TReplicatedTableNode — реплицированные таблицы;
    // - THunkStorageNode — хранилище hunk'ов (большие значения отдельно от строк).

 //    - Tablet owner — объект мастера (таблица), который владеет логическими шардами-таблетами. Он отвечает на вопрос «чьи это данные».
 // - Tablet cell — группа tablet node'ов (Hydra-cell вне мастера), которая физически обслуживает таблеты: держит их в памяти, принимает запросы, делает динамические store'ы и компакшены. Отвечает на вопрос
 // «где эти таблеты сейчас крутятся».

    //  Tablet owner — любой объект мастера, который владеет таблетами (шардами). Базовый класс TTabletOwnerBase,
