...
///
/// @gearonixx
/// Все методы этого интерфейса должны вызываться только из одного потока (обычно из контрол-инвокера компонента, который этим сервером владеет).
/// Внутри реализация не делает локов, рассчитывая на это.

// В YT такие комментарии — стандартный способ документировать threading-контракт:
/*!
