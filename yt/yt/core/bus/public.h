...
// Default — обычные запросы, без особых требований. Control — служебка (heartbeat'ы между нодами, координация кластера); должна проходить даже когда всё забито. Heavy
// — толстые передачи (чанки данных), терпит латентность но жрёт полосу.
// RealTime — самый приоритетный, пускают вперёд всех (используется там, где задержка критична).
