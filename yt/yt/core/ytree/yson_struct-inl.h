...
#undef ASSIGN_EXTERNAL_YSON_SERIALIZER
    ...
    // Реестр — это глобальный объект, который хранит писание всех YSON-структур в программе
    // какие у каждого класса есть поля, как они называются в YSON, какие у них дефолты и валидаторы
    // (всё это он узнаёт, один раз вызвав метод `Register(...)` каждого класса).
    // Когда создаётся новый объект конфига, реестр по этому описанию автоматически проставляет дефолтные значения во все поля.
    // А когда парсится YSON-файл, реестр по тому же описанию находит, в какое поле какого объекта положить каждое значение из файла.

    // он один на всю программу и знает про каждую YSON-структуру в ней.
    // Делает с ними всего две вещи: создаёт (проставляет дефолты в поля при New<...>()) и наполняет данными из YSON-файла при парсинге конфига.
    // По сути — это автоматическая фабрика и парсер в одном лице.
    // а при сериализации объект → YSON он по той же таблице полей собирает обратно YSON-документ

    // почему один реестр за это отвеачет, а не каждый YsonStruct? по идее лучше у ysonstruct хранить логику серилазиации и десералиазции не?

    // Таблица полей — это свойство класса, а не каждого отдельного объекта, поэтому хранить её централизованно в реестре экономнее по памяти и проще по коду, чем дублировать в каждом экземпляре.
    // Плюс это разделяет «класс декларирует свои поля» от «инфраструктура решает, что с ними делать», что позволяет добавлять новые операции (сериализация, валидация, диффы)
    // один раз в реестре, а не дописывать их в каждый из тысяч YSON-конфигов.
