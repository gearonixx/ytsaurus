...
// Claude responded: Master — транзакцию ведёт мастер, нужна для метаданных (создать таблицу, поменять права) и записи в статические таблицы.Master — транзакцию ведёт мастер, нужна для метаданных (создать таблицу, поменять права) и записи в статические таблицы. Tablet — транзакцию ведёт tablet cell, нужна для записи в динтаблицы (быстрый путь, без мастера).
