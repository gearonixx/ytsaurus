...
///
///
/// **Batch** — это все запросы которые ты хочешь отправить, **subbatch** — это кусок батча, который влезает в один RPC-вызов (≤100 запросов), чтобы не перегружать мастер одним огромным запросом.

// максимум 100 запросов в одном подбатче
static constexpr int DefaultSubbatchSize = 100;

// максимум 10 таких подбатчей летят параллельно одновременно
static constexpr int DefaultMaxParallelSubbatchCount = 10;

// Таблеты Sequoia могут быть в процессе перемонтирования / миграции — временная недоступность
// Sequoia — это архитектурный проект внутри YTsaurus, цель которого — разгрузить мастер-сервер путём выноса части метаданных из in-memory Cypress в обычные динамические таблицы.
