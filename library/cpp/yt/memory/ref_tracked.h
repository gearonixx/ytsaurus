...
// @gearonixx
// Числовой ID типа в системе учёта памяти YT. Когда heap profiler показывает "класс `TFoo` сейчас держит 500 МБ"
// внутри это работает не по имени класса, а по интовому cookie, присвоенному `TFoo` один раз. Cookie быстрее сравнивать и хранить, чем строки.

// "Cookie" в системном программировании — устоявшийся термин для "непрозрачного идентификатора,
// который ты получаешь от системы и потом возвращаешь обратно, чтоб…"Cookie" в системном программировании
// устоявшийся термин для "непрозрачного идентификатора, который ты получаешь от системы и потом возвращаешь обратно, чтобы она тебя узнала".
