...
// @gearonix
// Умный указатель YT с встроенным в объект счётчиком ссылок — аналог std::shared_ptr, но счётчик лежит внутри самого объекта
// . TRefCounted — базовый класс, который держит счётчик внутри объекта.
// TIntrusivePtr<T> — обёртка-указатель, которая управляет этим счётчиком (инкрементит при копировании, декрементит при разрушении, удаляет объект на нуле).
