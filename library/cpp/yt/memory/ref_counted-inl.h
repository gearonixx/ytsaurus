...
////////////////////////////////////////////////////////////////////////////////
///
    // ref_counted-inl.h — содержит реализации шаблонов и inline-функций, которые слишком большие, чтобы держать их прямо в .h.
    // @gearonixx
    // inline — это подсказка компилятору: «вместо того, чтобы делать вызов этой функции, подставь её тело прямо в место вызова».
//     .cpp файл видит только сам себя. Заголовок видят все, кто его инклюдит. Inline и шаблонам нужно, чтобы их видели все — поэтому они обязаны быть в заголовке, не в .cpp.
// -inl.h — это просто способ держать заголовок чистым (отдельно интерфейс, отдельно реализация), но физически это всё равно заголовок
   //  inline в .cpp сработает только для вызовов внутри этого же .cpp. То есть компилятор подставит тело там, где оно вызывается в том же файле, где определено.
